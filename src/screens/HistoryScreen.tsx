...
const CHART_FIELDS = ["field1", "field2", "field3", "field4", "field5"];
...
export default function HistoryScreen() {
  ...
  const loadHistory = useCallback(
    async (cfg: ThingSpeakConfig, numPoints: number) => {
      try {
    ...
    >
      ...
      <View style={styles.pillRow}>
        {POINT_OPTIONS.map((opt) => (
          ...
          >
            <Text
              style={[styles.pillText, points === opt && styles.pillTextActive]}
            >
              {opt} pts
            </Text>
          </TouchableOpacity>
